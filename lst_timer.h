...
public:
    ...
    void (*callback_func)(http_conn*);   // 任务回调函数，回调函数处理的客户数，释放资源、关闭连接
    http_conn* user_data;
...
public:
    ...
    void add_timer(util_timer* timer) 
    {
        ...
        // 如果目标定时器的超时时间小于当前链表中的所有定时器超时时间，则把该定时器插入链表头部
        // 作为链表新的头节点，否则需要调用重载函数add_timer()，把它插入链表中合适的位置，
        // 以保证链表的升序特性
...
private:
    ...
    // 该函数表示将目标定时器timer添加到节点head之后的链表中
    void add_timer(util_timer* timer, util_timer* head) 
    {
        util_timer* prev = head;
        util_timer* tmp = prev->next;
        // 遍历 list_head 节点之后的部分链表，直到找到一个超时时间大于目标定时器的超时时间节点，并将目标定时器插入该节点之前
        while(tmp) {
